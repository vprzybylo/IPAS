...
def collect_clusters(monomers1, monomers2, rand_orient=False):
    ...
    '''----START ICE-ICE COLLECTION ------'''

    for n in range(len(monomers1)):
        cluster1 = monomers1[n]
        cluster2 = monomers2[n]
        
        cluster1.og_points = cluster1.points
        cluster1.og_rot = cluster1.rotation
        cluster2.og_points = cluster2.points
        cluster2.og_rot = cluster2.rotation
        
        cluster1.recenter()
        cluster2.recenter()
#        cluster1.rotate_to([0,0,0])         
        cluster1.plot_ellipsoid(cluster1, view='x')
#        cluster1.plot_ellipsoid(cluster1, view='y')
#         print(cluster1.rotation)
#        cluster1.rotate_to([0,1.57,0])
#         print(cluster1.rotation)
...
#         rx, ry, rz = sorted([rx,ry,rz], reverse=True)
#         rxs.append(rx)
#         rys.append(ry)
#         rzs.append(rz)
