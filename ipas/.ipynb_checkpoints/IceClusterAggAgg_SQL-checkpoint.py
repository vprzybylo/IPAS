...
from sqlalchemy import Column, Float, Integer, Boolean, ForeignKey, PickleType
from sqlalchemy.dialects.postgresql import ARRAY, DOUBLE_PRECISION
from sqlalchemy.ext.declarative import declarative_base
from base import Base

from ipas import IceCrystal as crys
print(crys.IceCrystal)#.__tablename__)
...
class IceCluster(Base):
    ...
    crystal_id = Column(Integer, ForeignKey('crystals.id'),nullable=False, index=True)
    #crystals = relationship(crys, back_populates="aggregates")

    #crystals = relationship(crys.IceCrystal, backref='aggregates')
    ...
    def _rotate_mat(self, mat):
        ...
        self.points['x'][:self.ncrystals] = points['x'][:self.ncrystals] * mat[0, 0] + points['y'][:self.ncrystals] * mat[0, 1] + points['z'][:self.ncrystals] * mat[0, 2]
        self.points['y'][:self.ncrystals] = points['x'][:self.ncrystals] * mat[1, 0] + points['y'][:self.ncrystals] * mat[1, 1] + points['z'][:self.ncrystals] * mat[1, 2]
        self.points['z'][:self.ncrystals] = points['x'][:self.ncrystals] * mat[2, 0] + points['y'][:self.ncrystals] * mat[2, 1] + points['z'][:self.ncrystals] * mat[2, 2]
