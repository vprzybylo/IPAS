...
from sqlalchemy import Column, Float, Integer, Boolean,ForeignKey
...
class IceCrystal(Base):
    ...
    #aggs  = relationship(IceCluster, back_populates="crystals")
    
    '''
    agg_phi = Column(Float, ForeignKey('aggregates.agg_phi'))
    agg_r = Column(Float, ForeignKey('aggregates.agg_r'))
    ncrystals = Column(Float, ForeignKey('aggregates.ncrystals'))

    agg_phi_rel = relationship('IceCluster', foreign_keys=agg_phi, uselist=True)
    agg_r_rel = relationship('IceCluster', foreign_keys=agg_r, uselist=True)
    ncrystals_rel = relationship('IceCluster', foreign_keys=ncrystals, uselist=True)
    '''
    ...
    def __init__(self, length, width, rand_orient=False, center=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        #child = relationship("Child", uselist=False, back_populates="length")
