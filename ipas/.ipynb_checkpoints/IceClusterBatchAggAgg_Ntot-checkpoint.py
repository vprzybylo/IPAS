...
class IceClusterBatch():
    ...
    def __init__(self, ncrystals1, ncrystals2, length, width, numaspectratios, cplxs, phi,
                        major_axis, minor_axis, req, d2, real_phi, phi2D, plates):

        self.ncrystals1=ncrystals1
        self.ncrystals2=ncrystals2
        self.length = length
        self.width = width
        if plates is None:
    ...
    def get_characteristics(self, var, ch_dist, bins=70, filename=None, ext='png',save = False, verbose=False):
        ...
        self.ch = np.ones((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.shape = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.dphigam = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.poserr = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.negerr = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.min_data = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.max_data = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)
        self.mean = np.zeros((self.ncrystals1+self.ncrystals2-1), dtype=np.float64)


        for i in range(self.ncrystals1+self.ncrystals2-1): #starting at 0 index
            if var == 'phi':
            ...
            if var == 'phi':
                self.dphigam[i] = np.log(self.ch[i]) - np.log(self.phio)
