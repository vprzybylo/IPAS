...
def change_in_density_nm(dds_Ntot_rand, dds_Ntot_flat, savefig=False):
    ...
    phiarr = [0.01, 0.1, 0.5, 1.0, 2.0, 10.0, 100.0]
    Ns = np.arange(9)
    for N in Ns:
        ax1.scatter(
            phiarr,
            np.mean(dds_Ntot_rand[:, 0, :, N], axis=1),
            c=cmap(norm(N), 8),
            s=200,
        )
    ...
    cbar = plt.colorbar(cb, ticks=np.linspace(0, 9, 10) + 0.5, format="%d", cax=cax)
    ...
    for N in Ns:
        ax2.scatter(
            phiarr,
            np.mean(dds_Ntot_flat[:, 0, :, N], axis=1),
            c=cmap(norm(N), 8),
            s=200,
        )
    ...
    if savefig:
        plt.savefig(
            "../plots/dd_nmono_bothorientations.pdf",
            format="pdf",
            dpi=300,
            bbox_inches="tight",
        )
...
def change_in_density_aggr(
    ...
    for N in Ns:
        agg_r = np.round(
            np.power(
                (
                    np.power(np.mean(agg_as_Ntot_rand[:, 0, :, N], axis=1), 2)
                    * np.mean(agg_cs_Ntot_rand[:, 0, :, N], axis=1)
                ),
                (1.0 / 3.0),
            )
        )
        ax1.scatter(
            phiarr, agg_r, c=cmap(norm(np.mean(n_perc[:, 0, :, N], axis=1))), s=N * 50
        )
    ...
    for N in Ns:
        agg_r = np.round(
            np.power(
                (
                    np.power(np.mean(agg_as_Ntot_flat[:, 0, :, N], axis=1), 2)
                    * np.mean(agg_cs_Ntot_flat[:, 0, :, N], axis=1)
                ),
                (1.0 / 3.0),
            )
        )
        ax2.scatter(
            phiarr,
            agg_r,
            c=cmap(norm(np.mean(dds_Ntot_flat_perc[:, 0, :, N], axis=1))),
            s=N * 50,
        )
...
def dd_rand_hist(dds_iceice_rand, dds_iceagg_rand, dds_aggagg_rand, savefig=False):
    ...
    binwidth = 0.1
    mindata = min(
        np.amin(dds_iceice_rand), np.amin(dds_iceagg_rand), np.amin(dds_aggagg_rand)
    )
    maxdata = max(
        np.amax(dds_iceice_rand), np.amax(dds_iceagg_rand), np.amax(dds_aggagg_rand)
    )
    bins = np.arange(mindata, maxdata + binwidth, binwidth)
    _, n, bins = plt.hist(
        dds_aggagg_rand.ravel(),
        color="blue",
        alpha=0.5,
        bins=100,
        label="agg-agg",
...
def violin_density_plots(dds_iceice_flat, dds_iceagg_flat, dds_aggagg_flat):
    ...
    colors = ["g", "b", "r"]
    dataset = [
        dds_iceice_flat.ravel(),
        dds_iceagg_flat.ravel(),
        dds_aggagg_flat.ravel(),
    ]
    for c, data in enumerate(dataset):
        data = data[(data < np.quantile(data, 0.95)) & (data > np.quantile(data, 0.05))]
...
def phi_pdfs_ba_ca_rand(
    ...
    for phio in phios:
        iceagg = np.vstack(
            [phi_ba_iceagg_rand[phio, r, :], phi_ca_iceagg_rand[phio, r, :]]
        )
        kde_iceagg = st.gaussian_kde(iceagg)
        aggagg = np.vstack(
            [phi_ba_aggagg_rand[phio, r, :], phi_ca_aggagg_rand[phio, r, :]]
        )
        ...
        print(
            "%3.1f, %10.2f, %15.2f, %15s"
            % (phio, (oblates / 300) * 100, (prolates / 300) * 100, bigger)
        )
        ...
        axs[i].plot(
            np.linspace(0.0, 100, 10), np.linspace(0.0, 100, 10), "gray", "--", zorder=2
        )
        ...
        axs[i].set_title(
            "[%.3f-%.3f]" % (phi_bins_flat[phio], phi_bins_flat[phio + 1]),
            fontfamily="serif",
        )
    ...
    plt.colorbar(
        m,
        boundaries=np.linspace(0, 50, 11),
    ...
    plt.colorbar(
        c,
        boundaries=np.linspace(0, 30, 11),
        ax=axes.ravel().tolist(),
        aspect=20,
        orientation="vertical",
        pad=0.04,
...
def phi_pdfs_ba_ca_flat(
    ...
    for c, phio in enumerate(phios):
        ...
        iceagg = np.vstack(
            [phi_ba_iceagg_flat[phio, r, :], phi_ca_iceagg_flat[phio, r, :]]
        )
        kde_iceagg = st.gaussian_kde(iceagg)
        aggagg = np.vstack(
            [phi_ba_aggagg_flat[phio, r, :], phi_ca_aggagg_flat[phio, r, :]]
        )
        kde_aggagg = st.gaussian_kde(aggagg)
