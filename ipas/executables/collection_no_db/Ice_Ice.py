    ...
    for phi in range(len(phioarr)):
        for r in range(len(reqarr)):
            for n in range(nclusters):
                if use_dask:
                    output[phi, r, n] = dask.delayed(collect.collect_clusters_iceice)(
                        phioarr[phi], reqarr[r], ncrystals, rand_orient, plot=plot
                    )
                else:
                    agg_as[phi, r, n], agg_bs[phi, r, n], agg_cs[phi, r, n], phi2Ds[
                        phi, r, n
                    ], cplxs[phi, r, n], dds[
                        phi, r, n
                    ] = collect.collect_clusters_iceice(
                        phioarr[phi], reqarr[r], ncrystals, rand_orient, plot=plot
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=[100],
        required=True,
        help="monomer radius/radii",
        type=int,
    )
    parser.add_argument(
        "--aggregates",
        "-a",
        metavar="number of aggregates",
        required=True,
        type=int,
    ...
    parser.add_argument(
        "--monomers",
        "-m",
        metavar="number of monomers",
        required=False,
        default=2,
        help="number of monomers per aggregate",
        type=int,
    )
    parser.add_argument(
        "--rand",
    ...
    parser.add_argument(
        "--save",
        "-s",
        metavar="save aggregate attributes",
    ...
    parser.add_argument(
        "--filename",
        "-f",
        metavar="saving filename",
        help="filename to save data (include path from execution location)",
        type=str,
        default=False,
        required="--filename" in sys.argv,
    )
    parser.add_argument(
        "--use_dask",
        metavar="parallelization",
    ...
    if use_dask:
        ...
        agg_as, agg_bs, agg_cs, phi2Ds, cplxs, dds = compute(
            phioarr, reqarr, nclusters, ncrystals, rand_orient
        )
