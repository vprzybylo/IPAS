...
class ClusterCalculations(plot.PlotCluster, clus.Cluster):
    ...
    def fit_ellipsoid(self, tol=0.01):  # mve = minimum volume ellipse
        ...
        A = (
            la.inv(
                ...
                - np.multiply.outer(self.centroid, self.centroid)
            )
            / d
        )
    ...
    def ellipsoid_axes_lengths(self, A):
        ...
        self.a, self.b, self.c = sorted([rx, ry, rz], reverse=True)
        return rx, ry, rz
    ...
    def overlap(self, new_crystal, seedcrystal):
        ...
        rel_area = (
            self.projectxy().buffer(0).intersection(new_crystal.projectxy().buffer(0))
        )
        ...
        pctovrlp = (
            rel_area.area / (self.projectxy().area + new_crystal.projectxy().area)
        ) * 100
    ...
    def complexity(self):
        ...
        try:
            multipt = [
                geom.MultiPoint(self.points[n][["x", "y"]])
