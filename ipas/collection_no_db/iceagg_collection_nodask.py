...
def collect_clusters_iceagg(phio, r, nclusters, ncrystals, rand_orient):
    ...
    for n in range(nclusters):
        ...
        while cluster.ncrystals < ncrystals:
            ...
            Ve_clus = 4.0 / 3.0 * np.pi * rx * ry * rz

            # a and c of monomers in cluster (all identical)
            a_clus = np.power(
                (np.power(cluster.mono_r, 3) / cluster.mono_phi), (1.0 / 3.0)
            )
            ...
            a_mono = np.power((np.power(crystal2.r, 3) / crystal2.phi), (1.0 / 3.0))
            ...
            Ve3 = (
                4.0 / 3.0 * np.pi * agg_a * agg_b * agg_c
