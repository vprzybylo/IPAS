...
class PlotCluster(clus.Cluster):
    ...
    def plot_ellipsoid_axes(self, ax, xell, yell, zell):
        ...
        ax.plot(xell[0, :], yell[0, :], zell[0, :], color="b", marker="o")
        ax.plot(xell[20, :], yell[20, :], zell[20, :], color="b", marker="o")

        ax.plot(xell[10, :], yell[10, :], zell[10, :], color="r", marker="o")
        ax.plot(xell[30, :], yell[30, :], zell[30, :], color="r", marker="o")
    ...
    def plot_ellipsoid_aggs(
        self,
        clusters,
        nearest_geoms_xz=None,
        nearest_geoms_yz=None,
        nearest_geoms_xy=None,
        view="x",
        circle=None,
        agg_agg=True,
        add_ellipsoid=True,
        ...
        # PLOT MONOMERS
        if agg_agg:
            start_list = [cluster.ncrystals for cluster in clusters]
            start = [0] + start_list
            end = [
                (np.sum([start[i], start[i + 1]]) + 1) for i in range(len(start_list))
            ]
            ...
            for cluster in range(len(clusters)):
                ...
                for crys in range(start[cluster], end[cluster] - 1):
                    self._plot_crystal(crys, ax, color)
        else:
            start_list = [cluster.ncrystals - 1 for cluster in clusters]
            ...
            for cluster in range(len(clusters)):
                ...
                # color = list(np.random.choice(range(10), size=3)/10)
                color = colors[cluster]
                for crys in range(start[cluster], end[cluster]):
                    self._plot_crystal(crys, ax, color)

        if nearest_geoms_xz is not None:
            if view == "x":
                ax.scatter(
                    nearest_geoms_xz[0].x,
    ...
    def ellipse_vertices_rotated(self, params, angle, leftverticex, rightverticex):
        ...
        xleft = (
            (leftverticex - params["xy"][0]) * np.cos(angle)
            - (leftverticey - params["xy"][1]) * np.sin(angle)
        ) + params["xy"][0]

        xright = (
            (rightverticex - params["xy"][0]) * np.cos(angle)
            - (rightverticey - params["xy"][1]) * np.sin(angle)
        ) + params["xy"][0]

        yleft = (
            (leftverticex - params["xy"][0]) * np.sin(angle)
            + (leftverticey - params["xy"][1]) * np.cos(angle)
        ) + params["xy"][1]

        yright = (
            (rightverticex - params["xy"][0]) * np.sin(angle)
            + (rightverticey - params["xy"][1]) * np.cos(angle)
        ) + params["xy"][1]
    ...
    def plot_axes_ellipse(self, params, ax):
        ...
        x, y = self.ellipse_vertices_rotated(params, angle, leftverticex, rightverticex)
    ...
    def plot_ellipse(self, dims, add_axes=True):
        ...
        for l in range(len(dims)):
            for i in range(self.ncrystals):
                ...
                ax.plot(
                    x1, y1, color=color, zorder=2, linewidth="2"
                )  # edges of polygons
