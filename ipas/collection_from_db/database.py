...
class Database:
    ...
    def append_shape(self):
        ...
        self.df.loc[self.df["shape"] == "oblate", "agg_r"] = np.power(
            (np.power(self.df["a"], 2) * self.df["c"]), (1.0 / 3.0)
        )
        self.df.loc[self.df["shape"] == "prolate", "agg_r"] = np.power(
            (np.power(self.df["c"], 2) * self.df["a"]), (1.0 / 3.0)
        )
    ...
    def get_df_phi(self, phi_bins, i):
        ...
        return self.df[
            (self.df["agg_phi"] > phi_bins[i]) & (self.df["agg_phi"] < phi_bins[i + 1])
        ]
