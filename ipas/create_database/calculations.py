...
from ipas.create_database.plot import PlotCluster
from ipas.create_database.cluster import Cluster
...
class ClusterCalculations(PlotCluster, Cluster):
    ...
    def phi_2D_rotate(self):
        ...
        if np.isnan(phi).any():
            #print(ellipse, major_axis['y'], minor_axis['y'])
            self.plot_ellipsoid_aggs([self], view='w', circle=None)
