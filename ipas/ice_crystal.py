...
from sqlalchemy import Column, Float, Integer, Boolean, ForeignKey, String, ARRAY, PickleType
from sqlalchemy.orm import relationship

class IceCrystal(ipas.Base):
    ...
    def __init__(self, length, width, rand_orient=False, center=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.r = int(np.round(np.power((np.power(self.width,2)*self.length),(1./3.))))
